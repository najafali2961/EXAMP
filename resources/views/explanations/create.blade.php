{{-- 
@extends('layouts.app')

@section('content')
    <div class="container">
        <h1>Create Explanation</h1>
        <form action="{{ route('explanations.store') }}" method="POST">
            @csrf
            <div class="form-group">
                <label for="question_id">Select Question</label>
                <select class="form-control" id="question_id" name="question_id" required>
                    @foreach ($questions as $question)
                        <option value="{{ $question->id }}">
                            {{ $question->statement ? $question->statement->text : 'No text available' }}
                        </option>
                    @endforeach
                </select>
            </div>

            <div class="form-group">
                <label for="text">Explanation Text</label>
                <textarea class="form-control" id="text" name="text" required></textarea>
            </div>
            <button type="submit" class="btn btn-primary">Create</button>
        </form>
    </div>
@endsection --}}
@extends('layouts.app')

@section('content')
    <div class="container">
        <h1>Create Explanation</h1>
        <form action="{{ route('explanations.store') }}" method="POST">
            @csrf
            <div class="form-group">
                <label for="question_id">Select Question</label>
                <select class="form-control" id="question_id" name="question_id" required>
                    @foreach ($questions as $question)
                        <option value="{{ $question->id }}">
                            {{ $question->statements->first()->text ?? 'No text available' }}
                        </option>
                    @endforeach
                </select>
            </div>

            @foreach ($languages as $language)
                <div class="form-group">
                    <label for="text_{{ $language->code }}">Explanation Text ({{ $language->name }})</label>
                    <textarea class="form-control" id="text_{{ $language->code }}" name="text_{{ $language->code }}" required></textarea>
                </div>
            @endforeach

            <button type="submit" class="btn btn-primary">Create</button>
        </form>
    </div>
@endsection
